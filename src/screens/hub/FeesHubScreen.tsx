  ...
  return (
    ...
    >
      ...
      <AppHubHeader
        ...
        subtitle={
          isParent || isStudent
            ...
            : 'Fee management and collections'
        }
        title="Fees"
      />
      ...
      >
        ...
        {isStaff && schoolId ? (
          <>
            <AppSectionLabel accent="#F26B55" title="Overview" />
            ...
            <AppSectionLabel accent="#18A978" title="Transactions" />
            <AppModuleCard accent="#18A978" description="Record and manage fee payments" icon="wallet" onPress={() => navigation.navigate(ROUTES.PAYMENTS, feeCtx)} tint="#E8F8F2" title="Payments" />
            <AppModuleCard accent="#18A978" description="View and share payment receipts" icon="file-text" onPress={() => navigation.navigate(ROUTES.RECEIPTS, feeCtx)} tint="#E8F8F2" title="Receipts" />
          </>
        ) : null}

        {canManageFees && schoolId ? (
          <>
            <AppSectionLabel accent="#F59A23" title="Billing" />
            <AppModuleCard accent="#F59A23" description="Generate monthly and one-time fee invoices" icon="refresh" onPress={() => navigation.navigate(ROUTES.GENERATE_FEE_DUES, feeCtx)} tint="#FFF4E4" title="Generate Fees" />
            <AppModuleCard accent="#F59A23" description="Pending and overdue fee dues list" icon="clock" onPress={() => navigation.navigate(ROUTES.PENDING_FEES, feeCtx)} tint="#FFF4E4" title="Pending Fees" />
          </>
        ) : null}

        {canSetupFees && schoolId ? (
          <>
            <AppSectionLabel accent="#7A5AF8" title="Configuration" />
